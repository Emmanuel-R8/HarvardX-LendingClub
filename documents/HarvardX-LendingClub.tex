...
\StringTok{    }\KeywordTok{select}\NormalTok{(PMT_delta)}
  ...
  \KeywordTok{round}\NormalTok{(}\KeywordTok{mean}\NormalTok{(}\DecValTok{100} \OperatorTok{*}\StringTok{ }\NormalTok{installmentError}\OperatorTok{$}\NormalTok{PMT_delta), }\DataTypeTok{digits =} \DecValTok{2}\NormalTok{)}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  lending_club <-}\StringTok{ }\KeywordTok{as_tibble}\NormalTok{(lending_club)}
  ...
  \CommentTok{# Leave the original dataset untouched and work with a copy.}
...
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{loanID =} \KeywordTok{row_number}\NormalTok{()) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }
...
\StringTok{    }
\StringTok{    }\CommentTok{# order by date}
\StringTok{    }\KeywordTok{arrange}\NormalTok{(issue_d) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }
...
\NormalTok{      schedule[i, }\StringTok{"monthlyPayment"}\NormalTok{] <-}\StringTok{ }\NormalTok{installment}
\NormalTok{      schedule[i, }\StringTok{"totalPandI"}\NormalTok{] <-}
\StringTok{        }\NormalTok{schedule[i }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{, }\StringTok{"totalPandI"}\NormalTok{] }\OperatorTok{+}\StringTok{ }\NormalTok{installment}
\NormalTok{      schedule[i, }\StringTok{"totalI"}\NormalTok{]     <-}
\StringTok{        }\NormalTok{schedule[i }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{, }\StringTok{"totalI"}\NormalTok{]   }\OperatorTok{+}\StringTok{ }\NormalTok{accruedInterest}
\NormalTok{      schedule[i, }\StringTok{"totalP"}\NormalTok{]     <-}
\StringTok{        }\NormalTok{schedule[i }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{, }\StringTok{"totalP"}\NormalTok{]   }\OperatorTok{+}\StringTok{ }\NormalTok{installment }\OperatorTok{-}\StringTok{ }\NormalTok{accruedInterest}
\NormalTok{    \}}
\NormalTok{  \}}
...
\NormalTok{  schedule[}\DecValTok{1}\NormalTok{, }\StringTok{"monthlyPayment"}\NormalTok{] <-}\StringTok{ }\OperatorTok{-}\NormalTok{loan}
  
  \ControlFlowTok{if}\NormalTok{ (showSchedule) \{}
\NormalTok{    schedule }\OperatorTok{%>%}\StringTok{ }\KeywordTok{view}\NormalTok{()}
\NormalTok{  \}}
  
\NormalTok{  NPV <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(interest, cashFlow) \{}
\NormalTok{    t =}\StringTok{ }\DecValTok{0}\OperatorTok{:}\NormalTok{(}\KeywordTok{length}\NormalTok{(cashFlow) }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{)}
    \KeywordTok{sum}\NormalTok{(cashFlow }\OperatorTok{/}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{interest) }\OperatorTok{^}\StringTok{ }\NormalTok{t)}
\NormalTok{  \}}
  
\NormalTok{  IRR <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(CF) \{}
\NormalTok{    res <-}\StringTok{ }\OtherTok{NA}
    \KeywordTok{try}\NormalTok{(\{}
\NormalTok{      res <-}\StringTok{ }\KeywordTok{uniroot}\NormalTok{(NPV, }\KeywordTok{c}\NormalTok{(}\OperatorTok{-}\FloatTok{0.9}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{cashFlow =}\NormalTok{ CF)}\OperatorTok{$}\NormalTok{root}
\NormalTok{    \},}
    \DataTypeTok{silent =} \OtherTok{TRUE}\NormalTok{)}
    \KeywordTok{return}\NormalTok{(res)}
\NormalTok{  \}}
  
  \KeywordTok{return}\NormalTok{(}\KeywordTok{tibble}\NormalTok{(}
    \DataTypeTok{loanID =}\NormalTok{ loanNumber,}
    \DataTypeTok{IRR =} \KeywordTok{round}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{IRR}\NormalTok{(}
\NormalTok{      schedule}\OperatorTok{$}\NormalTok{monthlyPayment}
\NormalTok{    ) }\OperatorTok{*}\StringTok{ }\DecValTok{12}\NormalTok{), }\DataTypeTok{digits =} \DecValTok{4}\NormalTok{),}
    \DataTypeTok{monthDefault =}\NormalTok{ monthDefault}
\NormalTok{  ))}
\NormalTok{\}}

\NormalTok{loanNumberIRR <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(loanNumber) \{}
  \KeywordTok{require}\NormalTok{(tidyverse)}
  
\NormalTok{  l <-}\StringTok{ }\NormalTok{loans }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(loanID }\OperatorTok{==}\StringTok{ }\NormalTok{loanNumber)}
  \KeywordTok{calculateIRR}\NormalTok{(}
    \DataTypeTok{loanNumber =}\NormalTok{ l}\OperatorTok{$}\NormalTok{loanID,}
    \DataTypeTok{loan =}\NormalTok{ l}\OperatorTok{$}\NormalTok{funded_amnt, }\DataTypeTok{intRate =}\NormalTok{ l}\OperatorTok{$}\NormalTok{int_rate, }\DataTypeTok{term =}\NormalTok{ l}\OperatorTok{$}\NormalTok{term, }
    \DataTypeTok{totalPaid =}\NormalTok{ l}\OperatorTok{$}\NormalTok{total_pymnt, }\DataTypeTok{totalPrincipalPaid =}\NormalTok{ l}\OperatorTok{$}\NormalTok{total_rec_prncp, }\DataTypeTok{totalInterestPaid =}\NormalTok{ l}\OperatorTok{$}\NormalTok{total_rec_int, }
    \DataTypeTok{recoveries =}\NormalTok{ l}\OperatorTok{$}\NormalTok{recoveries, }\DataTypeTok{lateFees =}\NormalTok{ l}\OperatorTok{$}\NormalTok{total_rec_late_fee, }
    \DataTypeTok{showSchedule =} \OtherTok{TRUE}
\NormalTok{  )}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[numbers=left,,]
\CommentTok{# }
\CommentTok{# Calculate all the IRRs and month of default for all the loans. }
\CommentTok{# }\AlertTok{WARNING}\CommentTok{: This takes around a full day to run!!!!}
\CommentTok{# }
\CommentTok{# The actual data was generated by the Julia version, with cross-checks.}
\CommentTok{# Julia version takes about 150 sec on the same unoptimised code.}
\CommentTok{# }
\KeywordTok{local}\NormalTok{(\{}
\NormalTok{  loansIRR <-}
\StringTok{    }\NormalTok{loans }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{rowwise}\NormalTok{() }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{do}\NormalTok{(}\KeywordTok{calculateIRR}\NormalTok{(}\DataTypeTok{loanNumber =}\NormalTok{ .}\OperatorTok{$}\NormalTok{loanID,}
                    \DataTypeTok{loan =}\NormalTok{ .}\OperatorTok{$}\NormalTok{funded_amnt, }\DataTypeTok{intRate =}\NormalTok{ .}\OperatorTok{$}\NormalTok{int_rate, }\DataTypeTok{term =}\NormalTok{ .}\OperatorTok{$}\NormalTok{term,}
                    \DataTypeTok{totalPaid =}\NormalTok{ .}\OperatorTok{$}\NormalTok{total_pymnt, }\DataTypeTok{totalPrincipalPaid =}\NormalTok{ .}\OperatorTok{$}\NormalTok{total_rec_prncp,}
                    \DataTypeTok{totalInterestPaid =}\NormalTok{ .}\OperatorTok{$}\NormalTok{total_rec_int,}
                    \DataTypeTok{recoveries =}\NormalTok{ .}\OperatorTok{$}\NormalTok{recoveries, }\DataTypeTok{lateFees =}\NormalTok{ .}\OperatorTok{$}\NormalTok{total_rec_late_fee))}

  \KeywordTok{saveRDS}\NormalTok{(loansIRR, }\StringTok{"datasets/lending_club_IRRs.rds"}\NormalTok{)}
  
\NormalTok{  \})}
\end{Highlighting}
\end{Shaded}

\hypertarget{julia-code}{%
\subsection{Julia code}\label{julia-code}}

\hypertarget{translation-of-irr}{%
\subsubsection{Translation of IRR}\label{translation-of-irr}}
...
\CommentTok{####################################################################################################}
\CommentTok{##}
\CommentTok{## CREDIT MARGIN}
\CommentTok{##}
\CommentTok{## This method of approximating the credit margin is far less sophisticated than what FI's do.}
\CommentTok{##}
\CommentTok{## We need to calculate what the credit margin should be on a defaulted loan to get a nil NPV.}
\CommentTok{##}
\CommentTok{## On a risk free loan the CF will be P+I at risk-free on _both_ borrowing and lending sides.}
\CommentTok{##}
\CommentTok{##  On a defaulted loan, the CF will be:}
\CommentTok{##      borrowing unchanged = P&I at risk-free}
\CommentTok{##    and}
\CommentTok{##      lending P&I at (risk-free + credit margin) until before default, then recoveries+fees.}
\CommentTok{##}
\CommentTok{## The principal amortisation profile depends on the credit margin used. We will arbitrarily use}
\CommentTok{## 20% which is a conservative assumption.}
\CommentTok{##}
\CommentTok{## credit risk on that CF should be nil with the right margin when discounted at risk-free.}
\CommentTok{##}
\CommentTok{## The function is very similar to the IRR calculation.}
\CommentTok{##}

\NormalTok{using DataFrames, Roots}

\CommentTok{# number of monthly payments to model}
\CommentTok{# It exceeds 60 months in case recoveries on a 60-month loan takes the schedule after 60 months.}
\KeywordTok{const}\NormalTok{ nMonths = }\FloatTok{90}



\CommentTok{# Sculpt credit foncier profiles over 36 and 60 months at 20% per annum.}
\CommentTok{# The profile is expressed as percentage of loan amount}
\KeywordTok{function}\NormalTok{ CreateCreditFoncier(;n = }\FloatTok{36}\NormalTok{, riskFree = }\FloatTok{0.0}\NormalTok{)}
...
\CommentTok{## New data frame to store the results}
\NormalTok{creditMargin_Result = DataFrame(loanID = zeros(}\DataTypeTok{Int64}\NormalTok{, nrow(lc)),}
\NormalTok{                                creditMargin = zeros(}\DataTypeTok{Float64}\NormalTok{, nrow(lc)),}
\NormalTok{                                monthDefault = zeros(}\DataTypeTok{Int64}\NormalTok{, nrow(lc)))}


\CommentTok{# ~150 sec. to do the whole dataset}
\NormalTok{@time }\KeywordTok{for}\NormalTok{ i }\KeywordTok{in} \FloatTok{1}\NormalTok{:nrow(lc)}
  \KeywordTok{global}\NormalTok{ creditMargin_Result}

  \CommentTok{# Use multiple-return-value}
  \CommentTok{# 1h17m runtime}
\NormalTok{  (creditMargin_Result[i, :loanID],}
\NormalTok{   creditMargin_Result[i, :creditMargin],}
\NormalTok{   creditMargin_Result[i, :monthDefault]) =}
\NormalTok{      CreditMargin(}
\NormalTok{          loanNumber = lc[i, :Column1],}
\NormalTok{          loan = lc[i, :funded_amnt], intRate = lc[i, :int_rate], term =lc[i, :tenor],}
\NormalTok{          totalPaid = lc[i, :total_pymnt], totalPrincipalPaid = lc[i, :total_rec_prncp],}
\NormalTok{          totalInterestPaid = lc[i, :total_rec_int],}
\NormalTok{          recoveries = lc[i, :recoveries], lateFees = lc[i, :total_rec_late_fee],}
\NormalTok{          riskFree = }\FloatTok{0.02}\NormalTok{,}
\NormalTok{          showSchedule = false)}
\KeywordTok{end}


\NormalTok{creditMargin_Result[}\FloatTok{1}\NormalTok{:}\FloatTok{10}\NormalTok{,:]}

\NormalTok{CSV.write(}\StringTok{"datasets/CreditMargins.csv"}\NormalTok{, creditMargin_Result)}
\end{Highlighting}
\end{Shaded}
