...
# saveRDS(listIV, "datasets/listIV20.rds")
# saveRDS(listBins, "datasets/listBins20.rds")
...
# saveRDS(list(data = loansBinning, bins = bestBins), "datasets/bestBins20.rds")
...
# saveRDS(allCategories, "datasets/allCategories20.rds")
...
{
  startTime <- proc.time()
  trainSGLM <- speedglm(isGoodLoan ~ ., data = loanSample, family = binomial(), intercept = TRUE)
...
{
  startTime <- proc.time()
  retrainSGLM <- speedglm(isGoodLoan ~ ., data = loanSample, family = binomial(), intercept = TRUE)
  cat(proc.time() - startTime)
}
...
loanMatrix <- loanSample %>%
  dplyr::select(-isGoodLoan) %>%
  as.matrix()
...
predictionMatrix <- predictionCategories %>%
  dplyr::select(-isGoodLoan) %>%
  as.matrix()
...
# Load the best variables and their `bins`
varBins <- readRDS("datasets/listBins20.rds")
bestVars <- readRDS("datasets/bestBins20.rds")
```
