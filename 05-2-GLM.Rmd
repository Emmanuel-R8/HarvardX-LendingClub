...
speedglm:::summary.speedglm(SGLM_B_train)$coefficients %>% 
  ...
  slice(1:10) %>% 
  select("Bin name") %>% 
  knitr::kable(caption = "First model. Example of NA coefficients.") %>% 
...
speedglm:::summary.speedglm(SGLM_B_train)$coefficients %>% 
  ...
  slice(1:15) %>%
  knitr::kable(caption = "First training. 15 best bins") %>% 
  kableExtra::kable_styling(latex_options = "HOLD_position")
...
speedglm:::summary.speedglm(SGLM_B_retrain)$coefficients %>% 
  ...
  select(Bin_name, Estimate, z_value) %>% 
  slice(1:15) %>%
  knitr::kable(caption = "Second training. 15 best bins") %>% 
  kableExtra::kable_styling(latex_options = "HOLD_position")
...
speedglm:::summary.speedglm(SGLM_B_reretrain)$coefficients %>% 
  ...
  select(Bin_name, Estimate, z_value) %>% 
  slice(1:15) %>%
  knitr::kable(caption = "Third training: 15 best bins") %>% 
  kableExtra::kable_styling(latex_options = "HOLD_position")
...
Table \@ref(tab:0502-training-criteria) shows the Akaike Information Criterion and Log-likelihood for each of the 3 training results. Two points to note:
...
tibble(
  ...
  knitr::kable(caption = "Training AIC and Log-likelihood") %>% 
  kableExtra::kable_styling(latex_options = "HOLD_position")
