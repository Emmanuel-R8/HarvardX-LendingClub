...
loansPredictors <-
  ...
  # [TODO] FOR THE MOMENT UNTIL MACRODATA IS FIXED
  # dplyr::select(-tidyselect::one_of("addr_state")) %>%
...
loss005 <- loansTraining %>% dplyr::select(principal_loss_pct) %>% dplyr::slice(sample005)
loss01 <- loansTraining %>% dplyr::select(principal_loss_pct) %>% dplyr::slice(sample01)
loss05 <- loansTraining %>% dplyr::select(principal_loss_pct) %>% dplyr::slice(sample05)
loss10 <- loansTraining %>% dplyr::select(principal_loss_pct) %>% dplyr::slice(sample10)
loss20 <- loansTraining %>% dplyr::select(principal_loss_pct) %>% dplyr::slice(sample20)

CM005 <- loansTraining %>% dplyr::select(creditMargin) %>% dplyr::slice(sample005)
CM01 <- loansTraining %>% dplyr::select(creditMargin) %>% dplyr::slice(sample01)
CM05 <- loansTraining %>% dplyr::select(creditMargin) %>% dplyr::slice(sample05)
CM10 <- loansTraining %>% dplyr::select(creditMargin) %>% dplyr::slice(sample10)
CM20 <- loansTraining %>% dplyr::select(creditMargin) %>% dplyr::slice(sample20)
```
...
Set <- loansWorkingSet %>%
  dplyr::select(purpose, funded_amnt, total_rec_prncp) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
Set <- loansWorkingSet %>%
  dplyr::select(addr_state, funded_amnt, total_rec_prncp) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
Set <- loansWorkingSet %>%
  dplyr::select(emp_length, funded_amnt, total_rec_prncp) %>%
  dplyr::mutate(emp_length = as.factor(ifelse(is.na(emp_length), 0, emp_length))) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
Set <- loansWorkingSet %>%
  dplyr::select(home_ownership, funded_amnt, total_rec_prncp) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
Set <- loansWorkingSet %>%
  dplyr::select(verification_status, funded_amnt, total_rec_prncp) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
Set <- loansWorkingSet %>%
  dplyr::select(application_type, funded_amnt, total_rec_prncp) %>%
  filter(total_rec_prncp < funded_amnt - 1) %>%
  dplyr::mutate(PrincipalLoss = (funded_amnt - total_rec_prncp) / funded_amnt)
...
```{r eval=FALSE}
{
  ...
  loansTmp <- loans005 %>% dplyr::select(-creditMargin, principal_loss_pct)
  trainRF <- train(
    x = loansTmp,
    y = CM005$creditMargin,
    method = "Rborist",
...
#
# Tuning parameter 'minNode' was held constant at a value of 3
# RMSE was used to dplyr::select the optimal model using the smallest value.
# The final values used for the model were predFixed = 32 and minNode = 3.
#
...
```{r eval=FALSE}
{
  ...
  loansTmp <-
    model005 %>% dplyr::select(-creditMargin, principal_loss_pct, monthDefault)
  trainXGB <- train(
    x = loansTmp,
    y = CM005$creditMargin,
    method = "xgbTree",
...
tmp01 <-
  loans %>%
  dplyr::slice(sample01) %>%
  dplyr::select(funded_amnt, int_rate, installment, term, total_pymnt, total_rec_prncp)
```
