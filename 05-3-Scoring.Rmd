...
# Score per variable
estimateScorecard <- allMatrix %*% ( GLMScores$points %>% as.matrix() ) 
...
estimateLogit <- allMatrix %*% estimateVector
estimateLogit <-
  enframe(estimateLogit[, 1]) %>%
...
loansTraining %>%
  cbind(estimateLogit) %>%
  ggplot(aes(estimateScorecard, col = grade)) +
  geom_density(adjust = 2) +
  scale_x_log10() + 
  scale_y_sqrt()
```
...
ROCRPrediction <- ROCR::prediction(predictionCoefficients$value, loansTest$isGoodLoan)
...
```{r}
probEstimates <- if_else(predictionCoefficients$value < 0, 0, predictionCoefficients$value)
