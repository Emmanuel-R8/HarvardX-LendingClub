...
The dataset represents a total of ca.$34bln in loan principals, which is a substantial share of the total amount stated to have been intermediated to date by LC (publicly reported to be $50bln+). About 55%/60% of the portfolio is fully repaid. See Table \@ref(tab:04-loan-per-status).
...
By comparison, we plot the 3-year (in red) and 5-year (in blue) bank swap rates in Figure \@ref(fig:04-swap-rates). We see that the swap curve has flattened in recent times (3-year and 5-y rates are almost identical). We also can see that in broad terms the interest rates charged reflect those underlying swap rates. It is therefore most relevant to examine the credit margins added to the swap rates.
...
When applying, a potential borrower must state the purpose of the loan. As shown in table \@ref(tab:04-loans-per-purpose), the main purpose is the consolidation of existing debts.
...
Figure \@ref(fig:04-loss-distribution-subrating) shows that for a given grade, the losses are very widely spread. The loans are group by ratings and loans that have been fully repaid are removed.
...
```{r 04-loss-distribution-subrating,fig.cap="Distribution of NPV (\\%) depending on rating \n(y-axis square-root scaling)"}
...
```{r 04-NPV-top,fig.cap="NPV \\% over 120\\% \n(no y-axis scaling)"}
NPVset <- 
  loansWorkingSet %>%
...
We here present other aspects of the dataset. The full list of variable is given in appendix (see Table \@ref(tab:08-variable-description)). This dataset will be reduced as we focused on our core question: _Are LC's loans priced appropriately?_.
...
In this reduced dataset, we focus on loans that have matured or been terminated. It contains  `r nrow(loans)` samples. Most of the loans (ca.80%) have been repaid in full. See Table \@ref(tab:04-matured-loans).
...
When grouped by grade (Figure \@ref(fig:04-funded-by-subgrade)), we see a clear correlation between grade and default: the lower the grade the higher the portion defaults (note the limited scale with a minimum at about 50%). In addition, most of the business is written in the B- or C-rating range.
