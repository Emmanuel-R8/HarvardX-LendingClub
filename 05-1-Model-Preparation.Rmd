...
__ WARNING: IF YOU RUN THE MODELING SECTION, YOU WILL HAVE: MEMORY USAGE OF UP TO 32GB, ALL A LOT DISK SWAPPING WHEN DATASETS ARE JUGGLED IN AND OUT OF MEMORY TO DISK TO MINIMISE MEMORY USAGE, AND CALCULATION TIMES BY BATCHES OF UP TO 10 MINUTES (ON MY LAPTOP). THIS EXCLUDES TIME NECESSARY TO PREPARE THE DATASET AS EXPLAINED ON THE PREVIOUS SECTIONS (THAT IS HOURS).__
...
```{r 05-01-loan-predictors,echo=TRUE,cache=FALSE}
## ######################################################################################
##
...
bestBins %>% 
  ...
  arrange(desc(IV)) %>% 
  slice(1:15) %>%
  knitr::kable(caption = "15 top variables by IV")
```
$$\text{ }$$
...
bestBins %>% 
  ...
  arrange(desc(IV)) %>% 
  slice(1:15) %>%
  knitr::kable(caption = "15 top relevant variables by IV")
```
$$\text{ }$$
...
bestBins %>% 
  ...
  arrange(IV) %>% 
  slice(1:15) %>%
  knitr::kable(caption = "15 worst variables by IV")
```
$$\text{ }$$
