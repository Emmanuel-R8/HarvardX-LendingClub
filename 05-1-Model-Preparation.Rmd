...
+ the raw LendingClub dataset (see @sec:lendingclub-dataset for how it was prepared);
...
\newline
```{r 05-01-loan-predictors,echo=TRUE}
## ######################################################################################
##
...
WOE and IV enable one to:
  ...
  + The theoretical background to the measure lies in information theory. It cannot be over-emphasized that the measures __do not__ use the values of the factors: it is measure only calculated using the binary GOOD/BAD values (see definitions below);
...
\newline
```{r 05-01-reinstall-binner,echo=FALSE,message=FALSE}
...
Table \@ref(tab:05-1-information-value-table) guidelines are recommended to the relevance of variables given their Information Value [@bokhari2019credit].

\newline
```{r 05-1-information-value-table}
tibble(
...
```{r 05-01-create-bestbins-10best,echo=TRUE,fig.cap="15 top variables by IV"}
